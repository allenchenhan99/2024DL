...
class Dataset_Dance(torchData):
    def __init__(self, root, transform, mode='test', video_len=7, partial=1.0):
        ...
        # data_num = len(glob(os.path.join(root, 'test_img/*')))
        ...
        # Debug: Check how many directories are being recognized
        data_num = len(glob(os.path.join(root, 'test_img/*')))
        print(f"Total directories found under test_img: {data_num}")
        
        for i in range(data_num):
            img_files = sorted(glob(os.path.join(root, f'test_img/{i}/*')), key=get_key)
            label_files = sorted(glob(os.path.join(root, f'test_label/{i}/*')), key=get_key)

            # Debug: Print out what is being found
            print(f"Images in folder {i}: {len(img_files)}")
            print(f"Labels in folder {i}: {len(label_files)}")
            
            if img_files and label_files:
                self.img_folder.append(img_files)
                self.label_folder.append(label_files)
            else:
                print(f"No images or labels found for folder index {i}")
    ...
    def __getitem__(self, index):
        ...
        if not frame_seq or not label_seq:
            raise IndexError("Image or label sequence is empty")

        imgs = [self.transform(imgloader(frame)) for frame in frame_seq]
        labels = [self.transform(imgloader(label)) for label in label_seq]
